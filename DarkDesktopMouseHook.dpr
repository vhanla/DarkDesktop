...
begin
  ...
  begin
    TargetWnd := FindWindow('DarkDesktopClass', nil);
    if TargetWnd <> 0 then
    begin
      New(Msg);
      Msg^.dwData := 0;
      Msg^.cbData := SizeOf(TMouseHookStruct);
...
begin
  ...
  begin
    ...
    then
    begin
      var TargetWnd := FindWindow('DarkDesktopClass', nil);
      if TargetWnd <> 0 then
      begin
